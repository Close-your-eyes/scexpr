...
'  group_vec <- unlist(purrr::pmap(list(x = SO, y = meta.col), function(x,y) x@meta.data[,y,drop=T]), use.names = F)
  ...
  scale.max <- as.numeric(format(brathering::floor2(max(wil_auc$avgExpr), 0.1), nsmall = 1))
  scale.min <- as.numeric(format(brathering::ceiling2(min(wil_auc$avgExpr), 0.1), nsmall = 1))
  ...
  if (length(fill) == 1 && fill == "auto") {
    fill <- colrr::col_pal(name = "RColorBrewer::RdBu", n = 11, direction = -1)[-c(1,11)]

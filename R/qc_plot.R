...
qc_plot2 <- function(SO,
  ...
  qc_p2 <- purrr::map2(stats::setNames(red_name, red_name), clust_name,  function(red_name, clust_name) {
    ...
    p3 <- composition_barplot(SO = SO,
                              x_cat = clust_name,
                              fill_cat = "orig.ident")[["plot"]] +
      ggplot2::theme_bw() +
      ...
      ggplot2::facet_wrap(ggplot2::vars("rel cluster comp"))

    return(patchwork::wrap_plots(p1, patchwork::wrap_plots(c(list(p3), p2), ncol = 2), nrow = 1, widths = c(0.4, 0.6)))
  })

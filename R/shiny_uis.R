...
#'
#' @examples
shiny_uis <- function(name = c("volcano_sc")) {

  name <- match.arg(name, c("volcano_sc"))
    ...
    ui <- shiny::fluidPage(
      ...
      shiny::fluidRow(
        ...
        shiny::column(2,
                      ...
                      shiny::fluidRow(shiny::selectInput(inputId = "geom1", label = "geom1", choices = as.character(formals(scexpr::.expr_jitter)$geom1)[-1], selected = as.character(formals(scexpr::.expr_jitter)$geom1)[-1][1])),
                      shiny::fluidRow(shiny::selectInput(inputId = "geom2", label = "geom2", choices = as.character(formals(scexpr::.expr_jitter)$geom2)[-1], selected = as.character(formals(scexpr::.expr_jitter)$geom2)[-1][1])),

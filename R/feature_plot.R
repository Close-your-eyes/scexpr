      ...
      ggplot2::guides(
        ...
        color = if (plot.colorbar) {
          ggplot2::guide_colorbar(barwidth = legend.barwidth,
                                  ...
                                  label.theme = ggplot2::element_text(size = legend.text.size, family = font.family),  # ggtext::element_markdown here locks barheight, bug
                                  title.theme = ggtext::element_markdown(size = legend.title.text.size, family = font.family),
                                  label.position = legend.label.position,
                                  title.position = legend.title.position,
                                   title = switch(plot.legend.title, legend.title, NULL))
...
.check.SO <- function(SO,
  ...
  equal_slots <- unlist(lapply(SO, function(x) identical(Seurat::GetAssayData(x, assay = assay, slot = "data"),
...
get_repel_coords <- function(.data, g_base, width=2, height=2, ...) {
  grid::grid.newpage()
  grid::pushViewport(grid::viewport(width = width, height = height))
  g <- g_base + ggrepel::geom_text_repel(ggplot2::aes(x, y), label = ".", data = .data, max.overlaps = Inf, ...)

  panel_params <- ggplot2::ggplot_build(g)$layout$panel_params[[1]]
  xrg <- panel_params$x.range
  yrg <- panel_params$y.range

  textrepeltree <- ggplot2::ggplotGrob(g) %>%
    grid::grid.force(draw = F) %>%
    grid::getGrob("textrepeltree", grep = T)
  children <- grid::childNames(textrepeltree) %>%
    grep("textrepelgrob", ., value = T)

  get_xy <- function(n) {
    grob <- grid::getGrob(textrepeltree, n)
    data.frame(
      x.repel = xrg[1] + diff(xrg) * grid::convertX(grob$x, "native", valueOnly = T),
      y.repel = yrg[1] + diff(yrg) * grid::convertY(grob$y, "native", valueOnly = T)
    )
  }

...
#' @param overwrite replace existing reduction.name
...
run_fft_tsne <- function(
  ...
  X <- SO@reductions[[reduction]]@cell.embeddings
  if (is.null(dims_in)) {
    dims_in <- 1:ncol(X)
  }
  X <- X[,dims_in]

  assay <- SO@reductions[[reduction]]@assay.used
  if (reduction.name %in% names(SO@reductions) && !overwrite) {
    stop(reduction.name, " exists in SO@reductions. set overwrite = T to replace.")
  }
  ...
  SO[[reduction.name]] <- SeuratObject::CreateDimReducObject(
    embeddings = Gmisc::fastDoCall(brathering::fft_tsne, args = params),
    assay = assay,
    key = reduction.key)
